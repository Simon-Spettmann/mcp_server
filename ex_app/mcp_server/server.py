...
class MCPServer:
    ...
    def __init__(
        self, name: str = "nextcloud-mcp-server", enabled_auth_methods: Optional[List[str]] = None
    ):
    ...
    async def initialize(self, nc_app: Optional[AsyncNextcloudApp] = None) -> FastMCP:
...
def create_mcp_server(
    nc_app: Optional[AsyncNextcloudApp] = None,
    enabled_auth_methods: Optional[List[str]] = None,
) -> FastMCP:
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        enabled_methods = [
            method for method, enabled in enabled_methods_config.items() if enabled
        ]
